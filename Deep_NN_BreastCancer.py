...
'''
Deep_NN_v1 notes: made a decision to move fit_model out of
deep_NN class and into Layers_class... see v2
'''
...
# breast cancer data: 30 features, target= 'malignant' or 'benign' (1s and 0s)
# dataset:     
    data = load_breast_cancer()
    all_features = data.data

# Since there are 30 features, I will try subsetting just 2 of them.    
    X = all_features
    y = data.target
...
# DEEP NETWORK (4 hidden layers) -- smallest loss: 3.71e-4    
    architecture = [ 
        {"input": 30, "output": 20, "activation": "tanh"},
...
#    plt.plot(x, track_loss[0.005])
    plt.plot(x, track_loss[0.0005])
    plt.title("Avg Loss per Iteration")
    plt.xlabel("45 values per Iteration")
    plt.ylabel("Avg Loss")
    plt.legend(['epsilon = 0.001', 'epsilon = 0.005', 'epsilon = 0.0001'], loc='upper right')
